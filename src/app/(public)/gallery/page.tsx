...
export default function GalleryPage() {
  return (
    <>
      <PageHero eyebrow="Gallery" title="A visual space ready for the real practice." description="Approved photography and video have not yet been supplied. The abstract studies below are clearly identified development placeholders, not images of Prabha Yogashala facilities, students, or events." />
      <Section>
        <Container>
          <div className="grid auto-rows-[18rem] gap-5 md:grid-cols-2 lg:grid-cols-12">
            {publicMedia.gallery.map((media, index) => (
              <figure
                key={media.src}
              ...
              >
                ...
                <figcaption className="absolute inset-x-4 bottom-4 flex items-center justify-between rounded-xl bg-background/90 px-4 py-3 backdrop-blur">
                  <span className="font-display text-lg text-brand-strong">{labels[index]}</span>
                  <Badge tone="neutral">Placeholder</Badge>
                </figcaption>
              </figure>
            ))}
          </div>
          <div className="mt-10 rounded-2xl border border-brand/10 bg-surface-subtle p-6 text-sm leading-6 text-muted sm:p-8">
